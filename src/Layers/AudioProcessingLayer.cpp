...
AudioProcessingLayer::AudioProcessingLayer(const Config &config)
    : config(config), 
      inputDevice(std::make_unique<GuitarIO::AudioDevice>()),
      outputDevice(std::make_unique<GuitarIO::AudioDevice>()),
      pitchDetector(
          std::make_unique<GuitarDSP::YinPitchDetector>(GuitarDSP::YinPitchDetector::Config{ 
              .threshold = 0.15f,
        ...
        for (const auto &device : outputDevices)
        {
            ...
            if (outputDevice->Open(device.id, outputConfig, OutputCallback, this))
            {
                if (outputDevice->Start())
                {
                    ...
                    LOG_INFO("Successfully opened output device: [{}] {} with {} channels", device.id, device.name, channels);
                    break;
                }
                else
                {
                    LOG_WARN("Failed to start output device [{}] {}: {}", device.id, device.name, outputDevice->GetLastError());
                    outputDevice->Close();
                }
            }
            else
            {
                LOG_WARN("Failed to open output device [{}] {}: {}", device.id, device.name, outputDevice->GetLastError());
