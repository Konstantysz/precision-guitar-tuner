...
std::optional<int> TuningPresets::FindClosestString(TuningMode mode, float frequency, float referencePitch,
                                                      float toleranceCents)
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        float centsDiff = std::abs(GuitarDSP::NoteConverter::FrequencyToCents(frequency, preset.targetFrequencies[i]));
